...
if (isset($_POST['simpan'])) {
    ...
    $queryInsert = "INSERT INTO surat_keluar (nomor_surat, tanggal, tujuan, perihal, isi) 
                    VALUES ('$nomor','$tanggal','$tujuan','$perihal','$isi')";
    if (mysqli_query($conn, $queryInsert)) {
        $_SESSION['pesan'] = "Surat keluar berhasil ditambahkan!";
    } else {
        $_SESSION['pesan'] = "Gagal menambahkan surat keluar: " . mysqli_error($conn);
    }
...
if (isset($_POST['update'])) {
    ...
    $update = "UPDATE surat_keluar SET 
                  nomor_surat='$nomor',
                  tanggal='$tanggal',
                  tujuan='$tujuan',
                  perihal='$perihal',
    ...
    if (mysqli_query($conn, $update)) {
        $_SESSION['pesan'] = "Surat keluar berhasil diperbarui!";
    } else {
        $_SESSION['pesan'] = "Gagal memperbarui surat keluar!";
    }
    header("Location: surat_keluar.php");
    exit;
}
...
if ($keyword != "") {
    ...
    $query = "SELECT * FROM surat_keluar ORDER BY tanggal DESC";
}
...
<head>
  ...
  <style>
    ...
    .sidebar-bg { background: linear-gradient(180deg, #dc2626, #b91c1c, #7f1d1d); }
  </style>
</head>
<body class="bg-gray-50 fade-in">
<!-- Sidebar -->
<div id="sidebar" class="fixed top-0 left-0 w-64 h-full sidebar-bg shadow-xl z-50 transform -translate-x-full transition-transform duration-300 ease-in-out flex flex-col text-white">
  ...
  <ul class="flex-1 mt-6 px-4 space-y-2">
    ...
    <li>
      <a href="surat_keluar.php" class="flex items-center p-3 rounded-xl relative group bg-white/10 hover:bg-white/20 transition font-semibold text-red-100">
        <span class="absolute left-0 top-0 h-full w-1 bg-red-400 rounded-r-lg scale-y-100 transition origin-top"></span>
        <i class="fas fa-paper-plane mr-3 text-red-300"></i>
  ...
  <div class="w-full flex items-center justify-between px-6 py-4 bg-red-600 shadow fade-in">
    <div class="flex items-center space-x-3">
      ...
      <h1 class="text-lg md:text-xl font-semibold text-white">Surat Keluar</h1>
    </div>
  ...
  <div class="max-w-6xl mx-auto mt-6 px-6 fade-in">
    <?php if(isset($_SESSION['pesan'])): ?>
      <div class="mb-4 p-3 rounded-lg transition
                  <?= strpos($_SESSION['pesan'], 'berhasil') !== false ? 'bg-red-100 text-red-700' : 'bg-red-200 text-red-900' ?>">
        <?= $_SESSION['pesan']; ?>
      </div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-gray-400">Kelola semua surat keluar</p>
      </div>
      <button id="openModal" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl shadow transition transform hover:scale-105">
        <i class="fa-solid fa-plus mr-1"></i> Tambah Surat Keluar
      </button>
    </div>

    <!-- Form Pencarian -->
    <form method="get" class="flex mb-6 fade-in">
      <input type="text" name="cari" placeholder="Cari berdasarkan nomor surat, perihal, atau tujuan..." value="<?=htmlspecialchars($keyword)?>" class="flex-1 border rounded-l-xl px-4 py-2 focus:outline-none focus:ring focus:ring-red-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-white border-2 border-red-600 rounded-xl p-6 shadow card-hover fade-in">
          <div class="flex items-center mb-3">
            <i class="fa-solid fa-paper-plane fa-2x text-red-600 mr-2"></i>
            <h5 class="text-lg font-bold"><?=$row['perihal']?></h5>
          </div>
          <p class="text-sm text-gray-500 mb-2">#<?=$row['nomor_surat']?></p>
          <p><i class="fa-solid fa-calendar-days text-red-600 mr-2"></i><b>Tanggal:</b> <?=date("d F Y", strtotime($row['tanggal']))?></p>
          <p><i class="fa-solid fa-location-dot text-red-600 mr-2"></i><b>Tujuan:</b> <?=$row['tujuan']?></p>
  ...
  <div id="modal" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50">
    <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 overflow-y-auto max-h-[90vh] fade-in">
      <h2 class="text-xl font-bold mb-4 text-red-600"><i class="fa-solid fa-plus"></i> Tambah Surat Keluar</h2>
      <form method="post">
        ...
        <div class="mb-3"><label class="block text-sm font-medium">Tujuan</label><input type="text" name="tujuan" required class="w-full border rounded-lg px-3 py-2"></div>
        ...
        <div class="flex justify-end gap-3 mt-4">
          <button type="button" id="closeModal" class="px-4 py-2 bg-gray-300 rounded-lg transition hover:bg-gray-400">Batal</button>
  ...
  <div id="editModal" class="modal fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50">
    <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 overflow-y-auto max-h-[90vh] fade-in">
      <h2 class="text-xl font-bold mb-4 text-red-600"><i class="fa-solid fa-pen-to-square"></i> Edit Surat Keluar</h2>
      <form method="post">
        ...
        <div class="mb-3"><label class="block text-sm font-medium">Tujuan</label><input type="text" name="tujuan" id="edit_tujuan" required class="w-full border rounded-lg px-3 py-2"></div>
        ...
        <div class="flex justify-end gap-3 mt-4">
          <button type="button" id="closeEditModal" class="px-4 py-2 bg-gray-300 rounded-lg transition hover:bg-gray-400">Batal</button>
